package com.ts.importing.adornerClasses
{
	...
	public class SignalCoordinationAdorner extends AbstractAdorner
	{
		public function SignalCoordinationAdorner()
		{
			super( "corridor" );
		}
		...
		override protected function buildPhaseDecoration(dataIndex:DataIndex, system:SystemProxy):Array
		{
			...
			var perdueProxy:PerdueProxy 	= facade.retrieveProxy( PerdueProxy.NAME ) as PerdueProxy;
			...
			for each( var intersection:IntersectionProxy in system.networkModel.intersectionIndex.intersections )
			{
				...
				{	
					...
					//-- create all Trend adorners
					//var historyGraphProxies:Array = new Array();
					
					
					
					//-- Perdue Proxy --------------------------------------------------------------------------------------------------
					var perdueDetectionTimes:DataProxy 		= dataIndex.getDataAtUniqueID( SignalCoordinationDataName.DETECTION_TIME, phase.signalNumber, intersection.intersectionNumber );
					var perdueDetectionVehicles:DataProxy 	= dataIndex.getDataAtUniqueID( SignalCoordinationDataName.DETECTION_VEH_NUM, phase.signalNumber, intersection.intersectionNumber );
					...
					//-- Perdue Proxy - validate
					if( perdueDetectionTimes && perdueDetectionVehicles && signalData )
					{
						if( !isNaN( perdueDetectionTimes.maxValue ) 	&& !isNaN( perdueDetectionTimes.minValue ) &&
							!isNaN( perdueDetectionVehicles.maxValue ) 	&& !isNaN( perdueDetectionVehicles.minValue ) &&
							!isNaN( signalData.maxValue ) 				&& !isNaN( signalData.minValue ) )
						{
							perdueProxy.addPhasePlot( phase.uniqueID, perdueDetectionVehicles, perdueDetectionTimes, signalData );
						}
					}
