package com.ts.importing.adornerClasses
{
	...
	public class RRPreemptionAdorner extends AbstractAdorner
	{
		public function RRPreemptionAdorner()
		{
		...
		override protected function buildPhaseDecoration(dataIndex:DataIndex, system:SystemProxy):Array
		{
			...
			for each( var intersection:IntersectionProxy in system.networkModel.intersectionIndex.intersections )
			{
				for each( var phase:PhaseProxy in intersection.phaseIndex.phases )
				{
					...
					//-- build all notification adorners
					
					//-- rrSequence Notification
					...
					var rrSequenceData:DiscreteDataProxy = dataIndex.getDataAtUniqueID( RailroadPreemptionDataName.PREEMPT_SEQUENCE_STATE, phase.signalNumber, intersection.intersectionNumber ) as DiscreteDataProxy;
					
					if( rrSequenceData )
					{
						var rrSequenceProxy:NotificationPlotProxy = new NotificationPlotProxy( [], [ new NotificationDecorationEntry( 1, 0x006CFF, 1 ),
																									 new NotificationDecorationEntry( 2, 0x7E00FF, 1 ),
																									 new NotificationDecorationEntry( 3, 0xFF00EA, 1 ),
																									 new NotificationDecorationEntry( 4, 0xFF7E00, 1 ) ] );
						
						notificationProxies.push( new NotificationProxy( rrSequenceData as DiscreteDataProxy, true, rrSequenceProxy ) );
					}
					else
					{
						notificationProxies.push( new NotificationProxy( null, false, null ) );
					...
					//--rrSequenceEvent Notification
					var rrSequenceEventData:DiscreteDataProxy = dataIndex.getDataAtUniqueID( RailroadPreemptionDataName.PREEMPT_SEQUENCE_EVENT, phase.signalNumber, intersection.intersectionNumber ) as DiscreteDataProxy;
					
					if( rrSequenceEventData )
					{
						var rrSequenceEventProxy:NotificationPlotProxy = new NotificationPlotProxy( [], [ new NotificationDecorationEntry( 1, 0x006CFF, 1 ),
							new NotificationDecorationEntry( 2, 0x7E00FF, 1 ),
							new NotificationDecorationEntry( 3, 0xFF00EA, 1 ),
							new NotificationDecorationEntry( 4, 0xFF7E00, 1 ) ] );
						
						notificationProxies.push( new NotificationProxy( rrSequenceEventData as DiscreteDataProxy, false, rrSequenceEventProxy ) );
					}
					else
					{
						notificationProxies.push( new NotificationProxy( null, false, null ) );
