...
int node::sumAncestorsVal() {
  int sum = 0;
  for(node *pTemp = this; pTemp != NULL; pTemp = pTemp->parent) {
    sum += pTemp->val;
  }
  return(sum);
}
...
int node::nAncestors(){
  int i = 0;
  for(node *pTemp = this; pTemp != NULL && !pTemp->isEmpty(); pTemp = pTemp->parent) {i++;}
  return(i);
}
...
node *node::sortedValLowHigh() {
  if(this == NULL || this->isEmpty()) return(this);

  node *pTemp = this;
  node *retNode = new node('0',0,0,0);
  retNode->popFront();
  while(!pTemp->isEmpty()) {
    //find min
    node *min = pTemp;
    for(node *nTemp = pTemp; nTemp != NULL; nTemp = nTemp->next) {
      if(nTemp->val < min->val && !retNode->exists(nTemp->x,nTemp->y)) {
	min = nTemp;
      }
    }
    //done find min
    retNode->addNode(min->name,min->val,min->x,min->y);
    if(pTemp->x == min->x && pTemp->y == min->y) {
      pTemp->popFront();
    }
  }
  return(retNode);
...
node *node::sortedValHighLow() {
  ...
  while(!pTemp->isEmpty()) {
    ...
    node *max = pTemp;
    for(node *nTemp = pTemp; nTemp != NULL; nTemp = nTemp->next) {
      if(nTemp->val > max->val && !retNode->exists(nTemp->x,nTemp->y)) {
	max = nTemp;
      }
    }
    //done find min
    retNode->addNode(max->name,max->val,max->x,max->y);
    if(pTemp->x == max->x && pTemp->y == max->y) {
      pTemp->popFront();
    }
  }
  return(retNode);
