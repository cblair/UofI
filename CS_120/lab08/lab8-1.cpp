...
{
	private:
		int list_0, list_1, list_2;
		int list_3, list_4, list_5;
...
SmList::SmList ()
	:list_0(0), list_1(0), list_2(0),
	 list_3(0), list_4(0), list_5(0),
	 list_size(0)
{}
...
//the first element of the list, list_0.
int SmList::front()
{
	if (empty()) 
	{
...
//Otherwise, returns the last list element, list_5.
int SmList::back()
{
	if (empty()) 
	{
...
int SmList::at(int i)
{
	  ...
	  cerr << "ERROR: SmList::at() - index number is out of range "
	       ...
	       << "list elements\n";
	if(i == 0) return (list_0);
	if(i == 1) return (list_1);
	if(i == 2) return (list_2);
	if(i == 3) return (list_3);
	if(i == 4) return (list_4);
	if(i == 5) return (list_5);
}
...
// Otherwise, the val for list_0 is overwritten by list_1, and
// the rest are scooted down. 0's are set at list_5 and move
// down, while list_size is decremented by 1. These work together
// to re-init. the list members and shrink the list. This funct.
// then returns how many elements are left, same as in the 
// previous function.
...
void SmList::clear()
{
  for(int cnt = 0; cnt < max_size(); ++cnt) set_at(cnt, 0);
