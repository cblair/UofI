...
void calc_spline_coefficients(SplineFunction* f)
{
   ...
   if (f->numpoints == 2)
   {
      f->b[0] = f->b[1] = (f->y[1]-f->y[0])/(f->x[1]-f->x[0]);
      f->c[0] = f->c[1] = 0.0;
      f->d[0] = f->d[1] = 0.0;
      return;
   }
 ...
 */
   ...
   if (f->numpoints > 3)
   {
      f->c[0] = f->c[2]/(f->x[3]-f->x[1]) - f->c[1]/(f->x[2]-f->x[0]);
      f->c[nm1] = f->c[nm2]/(f->x[nm1]-f->x[f->numpoints-3]) -
                  f->c[f->numpoints-3]/(f->x[nm2]-f->x[f->numpoints-4]);
      f->c[0] = f->c[0]*f->d[0]*f->d[0]/(f->x[3]-f->x[0]);
      f->c[nm1] = -f->c[nm1]*f->d[nm2]*f->d[nm2]/(f->x[nm1]-f->x[f->numpoints-4]);
   }
			  ...
			  double velocity, double acceleration)
{
    ...
    * (the binary search method doesn't work well if you at one of the
    * end points.
    */
